...
%% Load FTIR data
cd('FTIR_NbN_Spectra/Transmission');      
[wvlen1,Spec1,~]=LoadSpectra('NbN.spa',1);    % in percentage
cd(base_dir);   cd('FTIR_NbN_Spectra/Veemax/R30');      
[wvlen2,Spec2,~]=LoadSpectra('NbN.spa',1);    %Spec2=Spec2*1e-2;
cd(base_dir);   cd('FTIR_NbN_Spectra/Veemax/R40');      
[wvlen3,Spec3,~]=LoadSpectra('NbN.spa',1);    %Spec3=Spec3*1e-2;
cd(base_dir);   cd('FTIR_NbN_Spectra/Veemax/R50');      
[wvlen4,Spec4,~]=LoadSpectra('NbN.spa',1);    %Spec4=Spec4*1e-2;

%% Measurement Parameters
cd(base_dir);                         wv1 = linspace(wvlen1(1), wvlen1(end), 600);    
omega = 1.2398 ./ wv1;       angles = [0 30 40 50];                  
polarization = 45;                  thickness_nom = [9.93e-3, 500];    % in microns
sigma_thickness = [0.6e-3, 50];     sigma_angle = 1;                   % ±1° perturbation
...
for i = 1:n_trials
    ...
    ncal_all(i, :) = sqrt(x(1) - Drude + Lorentz);
end
...
function chi2_val = compute_chi2_fast(x, omega, wv1, R_data, n_sub, thickness, angles, pol, sigma_R)
    ...
    chi2_val = chi2 / (N * n_angles -6);  % reduced chi-squared
end